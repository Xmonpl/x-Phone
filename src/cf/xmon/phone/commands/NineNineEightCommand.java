...
public class NineNineEightCommand implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender cs, Command command, String s, String[] strings) {
        if (cs instanceof Player){
            if (strings.length == 0){
                return ChatUtils.sendMessage(cs, Main.getInstance().getConfig().getString("xmon.998$usage"));
            }
            ...
            if (((Player) cs).getItemInHand().getType() == Material.CLAY_BRICK && ((Player) cs).getItemInHand().getItemMeta().getDisplayName().equals(ChatUtils.fixColor(Main.getInstance().getConfig().getString("xmon.phone$title")))) {
                Bukkit.getOnlinePlayers().stream().forEach(x -> {
                    if (x.hasPermission("xmon.998")) {
                        if (((Player) x).getInventory().contains(Material.CLAY_BRICK)) {
                            ChatUtils.sendMessage((Player) x, Main.getInstance().getConfig().getString("xmon.998$sendto").replace("{MESSAGE}", sb.toString()).replace("{CORDS}", Main.getInstance().getConfig().getString("xmon.service$cords$template")
                ...
                return ChatUtils.sendMessage(cs, Main.getInstance().getConfig().getString("xmon.998$confirm"));
            }else{
                return ChatUtils.sendMessage(cs, Main.getInstance().getConfig().getString("xmon.998$phone$in$hand"));
            }
        }else{
